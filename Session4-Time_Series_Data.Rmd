...
stargazer(res1a, res1b, res1c, res1d, res1e, omit.stat = c("f", "rsq"), star.cutoffs = .05, notes = "*p<0.05", notes.append = FALSE, digits = 2, dep.var.caption = "", dep.var.labels = "Bachelors' Degrees (%)", covariate.labels = c("GDP Rate", "Homeownership", "Minimum Wage", "State Taxes", "Unemployment"), type = "html")
```
]
...
### What did you select from this list? Why?
...
.size23[
```{r, results='asis'}
stargazer(res1b, res1c, res1e, omit.stat = c("f", "rsq"), star.cutoffs = .05, notes = "*p<0.05", notes.append = FALSE, digits = 2, dep.var.caption = "", dep.var.labels = "Bachelors' Degrees (%)", type = "html", covariate.labels = c("Homeownership", "Minimum Wage", "Unemployment"))
```
]
...
- SLIDE: Rest of evaluation steps
...
### Ok, how well does the OLS fit the data?
...
.size50[
Make a marginal effects plot for each of the three predictor variables in the multiple regression.
]]

???

Make me a marginal effects plot for each of the predictors holding all other values at their mean or mode.
...
Here is a visualization of both the simple OLS result and the multiple.
...
```{r, fig.align='center', fig.retina=3, out.width='98%', fig.width=6, fig.asp=0.618, cache=TRUE}
tibble(

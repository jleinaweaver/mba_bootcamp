...
*Note: Just the intuitions today, deeper dive next class*
...
- Remember, the coefficients are the pieces we plug into the formula for a line.

<br>

- **SLIDE**: The standard error
...
**SLIDE**: Think of this like the standard deviation.
...
The standard deviation is a measure of the spread of a series of data points around the mean.

<br>

Here I am showing you two histograms for made-up data

- In both cases the distributions are somewhat normal around the average value.
...
The t-statistic is calculated by dividing the estimated coefficient by the standard error.

- In other words, the t-statistic rescales the coefficient to be in the terms of the SE.

- So, the education coefficient only has meaning in terms of years (e.g. 1 year of education increases earnings on average by $6.5k)

- BUT, the education t-value only has meaning in terms of the error (e.g. the education coefficient is 15.36 SEs away from zero).

<br>

### And why is the distance from zero useful for us to know?
- (The null hypothesis is a beta of zero!)

- The farther away from zero, the less likely the null hypothesis beats our alternative hypothesis

<br>

**SLIDE**: And how far from zero do we have to go to be "significant"?
...
We can visualize t-scores using the Student's t-distribution
...
- The red vertical line in the middle is the null hypothesis (beta = 0) and the shaded region is an estimate of possible t-values that are 95% possible IF THE NULL HYPOTHESIS IS TRUE.

<br>

So, in a world where education has ZERO effect on earnings we would expect our regression coefficient to have a t-value between -1.96 and +1.96

- That represents 95% confidence in this case.
